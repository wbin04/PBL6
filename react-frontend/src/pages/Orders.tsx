...
import { API, getImageUrl, formatPrice, formatDate, isAuthenticated, type Order } from '@/lib/api';
...
const Orders: React.FC = () => {
  ...
  const loadOrders = async () => {
    try {
      ...
      const delivered = ordersData.filter((o: Order) => o.status === 'Đã giao' || o.status === 'DELIVERED');
      await Promise.all(delivered.map(async (order: Order) => {
        try {
          const ratingList = await API.get(`/ratings/?order=${order.id}`);
          (order as any).is_rated = Array.isArray(ratingList) && ratingList.length > 0;
        } catch {
  ...
  const viewOrderDetail = async (orderId: number) => {
    try {
      ...
      alert('Không thể tải chi tiết: ' + (error instanceof Error ? error.message : String(error)));
    }
  };
  ...
  const cancelOrder = async (orderId: number) => {
    ...
    try {
      ...
      alert('Không thể hủy đơn hàng: ' + (error instanceof Error ? error.message : String(error)));
    }
  };
  ...
  const saveRating = async () => {
    ...
    try {
      ...
      alert('Không thể gửi đánh giá: ' + (error instanceof Error ? error.message : String(error)));
    }
  };
  ...
  const updateDetailOrder = async () => {
    ...
    try {
      ...
      const receiverName = (document.getElementById('detail-receiver_name') as HTMLInputElement)?.value.trim();
      const phoneNumber = (document.getElementById('detail-phone_number') as HTMLInputElement)?.value.trim();
      const shipAddress = (document.getElementById('detail-ship_address') as HTMLTextAreaElement)?.value.trim();
      const note = (document.getElementById('detail-note') as HTMLTextAreaElement)?.value.trim();
      ...
      alert('Lỗi cập nhật: ' + (error instanceof Error ? error.message : String(error)));
    }
  };
    ...
    <div className="max-w-6xl mx-auto mt-8 px-4">
      ...
      {orders.length === 0 ? (
        <Card className="text-center py-12">
          <CardContent>
            <h3 className="text-xl font-semibold mb-4 text-gray-800">Bạn chưa có đơn hàng nào</h3>
            <p className="text-gray-600 mb-6">Hãy khám phá thực đơn và đặt món yêu thích của bạn!</p>
            ...
            <Card key={order.id} className="hover:shadow-lg transition-shadow">
              <CardHeader className="bg-gray-50 border-b">
                <div className="flex justify-between items-center">
                  ...
                  <span className={`px-3 py-1 rounded-full text-xs font-bold uppercase ${getStatusClass(order.status)}`}>
              ...
              <CardContent className="p-4">
                ...
                <div className="mb-4">
                  {order.items.map(item => (
                    <div key={`${item.food.id}-${order.id}`} className="flex items-center gap-4 mb-3 p-2 bg-gray-50 rounded">
                      <img
                ...
                <div className="mb-4 text-sm text-gray-600">
                  <div><strong>Giao đến:</strong> {(order as any).ship_address}</div>
                  <div><strong>Người nhận:</strong> {(order as any).receiver_name} - {(order as any).phone_number}</div>
                </div>
                ...
                <div className="flex flex-wrap gap-2">
                  ...
                  {getStatusText(order.status) === 'Chờ xác nhận' && (
                    <Button
                      variant="destructive"
                      size="sm"
                  ...
                  {getStatusText(order.status) === 'Đã giao' && (
                    <>
                      <Button
                        variant="secondary"
                        size="sm"
          ...
          <div className="bg-white rounded-lg max-w-md w-full max-h-screen overflow-y-auto">
            <div className="p-6">
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <input
                    ...
                    defaultValue={(selectedOrder as any).receiver_name}
                    disabled={getStatusText(selectedOrder.status) !== 'Chờ xác nhận'}
                  />
                </div>
                ...
                <div>
                  ...
                  <input
                    ...
                    className="w-full p-3 border rounded"
                    defaultValue={(selectedOrder as any).phone_number}
                    disabled={getStatusText(selectedOrder.status) !== 'Chờ xác nhận'}
                  />
                </div>
                ...
                <div>
                  ...
                  <textarea
                    ...
                    rows={3}
                    className="w-full p-3 border rounded"
                    defaultValue={(selectedOrder as any).ship_address}
                    disabled={getStatusText(selectedOrder.status) !== 'Chờ xác nhận'}
                  />
                </div>
                ...
                <div>
                  ...
                  <textarea
                    ...
                    rows={2}
                    className="w-full p-3 border rounded"
                    defaultValue={(selectedOrder as any).note}
                    disabled={getStatusText(selectedOrder.status) !== 'Chờ xác nhận'}
                  />
                </div>
              </div>
              ...
              <div className="flex justify-end gap-2 mt-6">
                {getStatusText(selectedOrder.status) === 'Chờ xác nhận' && (
                  <Button onClick={updateDetailOrder}>
                    Lưu
                  </Button>
                )}

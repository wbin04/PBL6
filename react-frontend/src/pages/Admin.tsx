...
const Admin: React.FC = () => {
  ...
  // Dashboard
  const [stats, setStats] = useState({ totalCustomers: 0, totalFoods: 0, totalOrders: 0, totalStores: 0 });
  ...
  const loadDashboard = async () => {
      setLoading(true);
      try {
          const [c, f, o, s] = await Promise.all([
             API.get('/auth/admin/customers/?page=1').catch(()=>({total_customers:0})),
             API.get('/menu/admin/foods/?page=1').catch(()=>({total_foods:0})),
             API.get('/orders/admin/?page=1').catch(()=>({total_orders:0})),
             API.get('/stores/').catch(()=>({results:[]}))
          ]);
          setStats({ totalCustomers: c.total_customers||0, totalFoods: f.total_foods||0, totalOrders: o.total_orders||0, totalStores: s.results?.length||0 });
    ...
    <div className="flex h-screen bg-gray-50">
       ...
       <aside className="w-64 bg-white border-r shadow-sm hidden md:flex flex-col z-20">
          ...
          <nav className="flex-1 p-4 space-y-1 overflow-y-auto no-scrollbar">
              ...
              <SidebarItem id="popularFoodsReport" label="Món bán chạy" icon={PieChart} />
          </nav>
              ...
              <div className="max-w-7xl mx-auto pb-10">
                ...
                {activeSection === 'dashboard' && (
                    <div className="grid grid-cols-1 md:grid-cols-4 gap-6 animate-in fade-in">
                        {[
                            { title: 'Cửa hàng', val: stats.totalStores, color: 'blue', icon: StoreIcon },
                            { title: 'Khách hàng', val: stats.totalCustomers, color: 'green', icon: Users },
                            { title: 'Món ăn', val: stats.totalFoods, color: 'orange', icon: Utensils },
                            { title: 'Đơn hàng', val: stats.totalOrders, color: 'purple', icon: ShoppingBag }
                        ].map((s, i) => (
                            <Card key={i} className={`border-l-4 border-l-${s.color}-500 shadow-sm`}><CardContent className="p-6 flex justify-between items-center">
                                <div>
           ...
           <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
               <div className="bg-white rounded-xl shadow-lg w-full max-w-lg p-6 max-h-[90vh] overflow-y-auto">
                   ...
                   <form onSubmit={showAddPromoModal?handleAddPromo:handleUpdatePromo} className="space-y-4">
                       ...
                       <div className="flex items-center gap-2"><input type="checkbox" checked={showAddPromoModal?newPromo.is_active:selectedPromo!.is_active} onChange={e=>{const v=e.target.checked; showAddPromoModal?setNewPromo({...newPromo,is_active:v}):setSelectedPromo({...selectedPromo!,is_active:v})}}/> <label>Kích hoạt</label></div>
                       <div className="flex justify-end gap-2 mt-4"><Button type="button" variant="outline" onClick={()=>{setShowAddPromoModal(false);setShowEditPromoModal(false)}}>Hủy</Button><Button type="submit">Lưu</Button></div>
                   </form>
               </div>
           </div>
       )}

...
const ensureApiKey = () => {
  if (!GOOGLE_MAPS_API_KEY) {
    throw new Error(
      'Thiếu khoá EXPO_PUBLIC_GOOGLE_MAPS_API_KEY. Vui lòng cấu hình Google Maps API key trong môi trường Expo.'
    );
  }
};
...
const normalizeGoogleStatus = (status: string, errorMessage?: string) => {
  ...
  const humanReadableMessage = errorMessage || 'Không thể kết nối tới Google Maps API';
